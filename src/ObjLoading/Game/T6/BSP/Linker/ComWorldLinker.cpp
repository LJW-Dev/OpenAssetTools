...
namespace BSP
{
    ...
    bool ComWorldLinker::linkComWorld(ZoneAssetPools* T5AssetPool, std::string& bspName)
    {
        auto T5ComWorldAsset = T5AssetPool->GetAsset(T5::ASSET_TYPE_COMWORLD, bspName);
        ...
        T6ComWorld->name = m_memory.Dup(T5ComWorld->name);
